...
| 42 | Applied quadruped, spike A2 (training on Colab) | Playground's default PPO on `Go1JoystickRoughTerrain`, Tesla T4: 206M steps in 83.7 min (criterion ≤ 60 min ✗); the policy walks: 10 episodes of 20 s at a 0.5 m/s command on rough terrain, 0 falls, 0.468 m/s (✓). **Fail on time only**; the eval reward plateaus from ~140M steps (28.2 → 28.9). Found by the local smoke test before running: JAX must be pinned to 0.9.2 for brax 0.14.2 | – (Playground reset, keys 1000–1009) | No | `applied/mujoco-quadruped/results/go1-a2-results.json` |
| 43 | Applied quadruped, spike A3 (policy in TypeScript, walking in the browser) | Max |TS − JAX| = 1.65e-7 over 100 recorded observations; in the browser's MuJoCo (WASM, C engine) 10 episodes of 20 s at 0.5 m/s: flat 0 falls, 0.487 m/s; rough 0 falls, 0.478 m/s. **Pass**. Native C MuJoCo and the WASM build give identical outcomes on identical initial conditions | – (seeds 1000–1009) | No | `artifacts/experiments/applied-go1-a3.json` |
| 44 | Applied quadruped, spike A4 (GPU planner) | Policy-guided predictive sampling, 512 × 0.5 s per decision, MJX JAX backend, Tesla T4: median 172.6 ms per decision (≤ 100 ms ✗); 0 falls, 0.470 vs 0.472 m/s for the policy alone (≥ ✗, narrowly). The noise-free candidate won 100% of 10,000 decisions: with fresh noise at every step and the task reward's action-rate penalty, no perturbed candidate ever scored higher, so the planner reduced to the policy. **Fail**. Declared next step: smooth (low-frequency) perturbations and a task score (command tracking and falls) instead of the full shaped reward, on a faster GPU, as a new change | – (keys 1000–1009) | No | `applied/mujoco-quadruped/results/go1-a4-results.json` |
