...
## Phase B — integration (only if the spike passes; spec deltas added before implementing)

- [ ] B.1 Add spec deltas for continuous players, training, evaluation and registry, and validate the change
- [ ] B.2 Implement continuous players, DAgger training with regression labels, continuous evaluation metrics and registry support; verify with tests and a smoke study
- [ ] B.3 Run the 5-run racing test study; integrate the racing demo (view, network layout for the ensemble mean); update README and EXPERIMENTS.md; redeploy; archive `add-racing-game` and this change
